...
import { mockSpecies, mockPaginatedResult } from './mocks'
...
describe('speciesService', () => {
  ...
  describe('getAll', () => {
    it('fetches the first page by default', async () => {
      mock.onGet('/species').reply(200, [mockSpecies])
      const result = await speciesService.getAll()
      expect(result).toEqual(mockPaginatedResult([mockSpecies]))
    })
    ...
    it('filters results by search param when provided', async () => {
      ...
      expect(result.items).toEqual([mockSpecies])
    })
