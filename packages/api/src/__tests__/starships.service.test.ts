...
import { mockStarship, mockPaginatedResult } from './mocks'
...
describe('starshipsService', () => {
  ...
  describe('getAll', () => {
    it('fetches the first page by default', async () => {
      mock.onGet('/starships').reply(200, [mockStarship])
      const result = await starshipsService.getAll()
      expect(result).toEqual(mockPaginatedResult([mockStarship]))
    })
    ...
    it('filters results by search param when provided', async () => {
      ...
      expect(result.items).toEqual([mockStarship])
    })
